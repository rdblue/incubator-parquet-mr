...
public class VariableWidthRLEEncoder {
  ...
  public VariableWidthRLEEncoder(int initialCapacity, int pageSize, ByteBufferAllocator allocator) {
    if (DEBUG) {
  ...
  private BytePacker getPacker(int bitWidth) {
    Preconditions.checkArgument(bitWidth <= 32,
    ...
    BytePacker packer = packers[bitWidth - 1];
    if (packer == null) {
  ...
  private void writeRleRun() throws IOException {
    ...
    BytesUtils.writeZigZagVarInt(previousValue, baos);
  ...
  private int maxWidth(int[] integers) {
    int setBits = 0;
    for (int i = 0; i < integers.length; i += 1) {
      setBits |= integers[i];
    }
    return 32 - Integer.numberOfLeadingZeros(setBits);
  }
}
