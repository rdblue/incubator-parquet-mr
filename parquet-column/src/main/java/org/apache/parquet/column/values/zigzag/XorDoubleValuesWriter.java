 ...
 */
public class XorDoubleValuesWriter extends ValuesWriter {
  private final VariableWidthRLEEncoder exponentEncoder;
  private final VariableWidthLongRLEEncoder mantissaEncoder;
  private long lastMantissaBits = 0L;

  public XorDoubleValuesWriter(int initialCapacity, int pageSize, ByteBufferAllocator allocator) {
    this.exponentEncoder = new VariableWidthRLEEncoder(initialCapacity, pageSize, allocator);
    this.mantissaEncoder = new VariableWidthLongRLEEncoder(initialCapacity, pageSize, allocator);
  }
  ...
  public void writeDouble(double v) {
    try {
      ...
      exponentEncoder.writeInt(ZigZagTransforms.Doubles.encodeExponent(bits));
      long mantissa = ZigZagTransforms.Doubles.encodeMantissa(bits);
      mantissaEncoder.writeLong(lastMantissaBits ^ mantissa);
  ...
  public Encoding getEncoding() {
    return Encoding.XOR_FLOATING_POINT;
  }
