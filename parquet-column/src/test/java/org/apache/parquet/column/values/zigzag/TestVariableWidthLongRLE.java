...
public class TestVariableWidthLongRLE {
  ...
  public void testRandomData() throws Exception {
    ...
    VariableWidthLongRLEEncoder encoder = new VariableWidthLongRLEEncoder(
    ...
    for (int i = 0; i < numValues; i += 1) {
      encoder.writeLong(Math.abs(actual.nextLong() % Short.MAX_VALUE));
    }
    ...
    VariableWidthLongRLEDecoder decoder = new VariableWidthLongRLEDecoder(
        bytes.toInputStream());
    for (int i = 0; i < numValues; i += 1) {
      Assert.assertEquals(String.valueOf(i),
          Math.abs(expected.nextLong() % Short.MAX_VALUE), decoder.readLong());
    }
  ...
  public void testRandomDataWithRLE() throws Exception {
    ...
    for (int i = 0; i < numValues; i += 1) {
      ...
      encoder.writeLong(Math.abs(actual.nextLong() % 10));
    }
    ...
    for (int i = 0; i < numValues; i += 1) {
      Assert.assertEquals(String.valueOf(i),
          Math.abs(expected.nextLong() % 10), decoder.readLong());
    }
  ...
  public void testZigZagEncoding() throws Exception {
    ...
    ParquetProperties properties = ParquetProperties.builder().build();
    VariableWidthLongRLEEncoder normalEncoder = new VariableWidthLongRLEEncoder(
        properties.getInitialSlabSize(),
        properties.getPageSizeThreshold(),
        properties.getAllocator());
    ...
    for (int i = 0; i < numValues; i += 1) {
      // a small set of values ensures that there will be random RLE runs
      normalEncoder.writeLong(normal.nextLong() % LIMIT - Integer.MAX_VALUE);
    }

    long normalSize = normalEncoder.toBytes().size();

    VariableWidthLongRLEEncoder zigzagEncoder = new VariableWidthLongRLEEncoder(
        properties.getInitialSlabSize(),
        properties.getPageSizeThreshold(),
        properties.getAllocator());

    for (int i = 0; i < numValues; i += 1) {
      zigzagEncoder.writeLong(zigzag(zigzag.nextLong() % LIMIT - Integer.MAX_VALUE));
    }

    BytesInput bytes = zigzagEncoder.toBytes();
    System.err.println("Normal size: " + normalSize);
    System.err.println("Compression ratio: " +
        ((double) normalSize) / 8 / numValues);
    ...
    for (int i = 0; i < numValues; i += 1) {
      Assert.assertEquals(String.valueOf(i),
          zigzag(expected.nextLong() % LIMIT - Integer.MAX_VALUE), decoder.readLong());
    }
