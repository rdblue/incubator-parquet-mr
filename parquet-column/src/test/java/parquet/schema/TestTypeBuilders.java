...
public class TestTypeBuilders {
  ...
  public void testMapWithInt64KeyAnnotations() {
    ...
    typeList.add(new PrimitiveType(REQUIRED, INT64, "key"));
    typeList.add(new PrimitiveType(REQUIRED, INT64, "value"));
    GroupType map = new GroupType(REPEATED, "myMap", OriginalType.MAP, new GroupType(REPEATED, "map",
        OriginalType.MAP_KEY_VALUE,
        typeList));
    MessageType expected = new MessageType("mapParent", map);
    GroupType actual = Types.buildMessage().repeatedMap()
        .key(INT64)
        .requiredValue(INT64)
  ...
  public void testRequiredNestedList() {
    ...
    Type actual = Types.optionalList()
        .requiredList()
        .optional(DOUBLE).named("innerElement")
        .named("innerList")
        .named("myList");
